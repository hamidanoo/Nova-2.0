...
const handleMessage = async (sock, msg) => {
  try {
    ...
    body = (body || '').trim();
      ...
      // For offline.js Command
      await require('./commands/owner/offline.js').autoReply(sock, msg, from, isGroup);
...
let autoTypingConfig = { enabled: false };
if (fs.existsSync(CONFIG_PATH)) {
  autoTypingConfig = JSON.parse(fs.readFileSync(CONFIG_PATH));
}

if (autoTypingConfig.enabled && !msg.key.fromMe) {
